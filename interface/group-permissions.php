		...
		<?php foreach ( $content_types as $index => $pt ) :  ?>
			...
			<div id="perm-panel-<?php echo $pt->name; ?>" class="perm-panel <?php echo $active; ?>" data-editable-original="<?php echo htmlspecialchars( implode( ',', $editable ) ); ?>">
				...
				<div id="perm-toolbar-<?php echo $pt->name; ?>-top" class="perm-toolbar top clearfix">
					<?php if ( $hierarchical ) :  ?>
					<p class="alignright">
						<a href="#" class="perm-tree-expand" data-target="perm-editor-<?php echo $pt->name; ?>"><?php _e( 'Expand All', 'bu-section-editing' ); ?></a> |
						<a href="#" class="perm-tree-collapse" data-target="perm-editor-<?php echo $pt->name; ?>"><?php _e( 'Collapse All', 'bu-section-editing' ); ?></a>
					</p>
					<?php else : ?>
					<p class="alignleft">
						<input id="perm-search-<?php echo $pt->name; ?>" type="text" name="perm-action[][search]" class="perm-search <?php echo $hiearchical_class; ?>" >
						<button class="perm-search flat button-secondary"><?php printf( __( 'Search %s', 'bu-section-editing' ), $pt->label ); ?></button>
					</p>
					<p class="alignright">
						<a class="perm-editor-bulk-edit" href="#" title="<?php esc_attr_e( 'Enable bulk edit mode', 'bu-section-editing' ); ?>"><?php _e( 'Bulk Edit', 'bu-section-editing' ); ?></a>
					</p>
					<?php endif; ?>
				...
				<div class="perm-editor-bulk-edit-panel clearfix">
					<div class="bulk-edit-actions">
						...
						<select name="perm-ed-bulk-edit[action]">
							<option value="none"><?php _e( 'Bulk Actions', 'bu-section-editing' ); ?></option>
							<option value="allowed"><?php _e( 'Allow selected', 'bu-section-editing' ); ?></option>
							<option value="denied"><?php _e( 'Deny selected', 'bu-section-editing' ); ?></option>
						</select>
						<button class="button-secondary"><?php _e( 'Apply', 'bu-section-editing' ); ?></button>
					</div>
				</div>
				<?php endif; ?>
				...
				<div class="perm-toolbar bottom clearfix">
					<div class="tablenav">
						<div id="perm-editor-pagination-<?php echo $pt->name; ?>" class="tablenav-pages">
							<span id=""class="displaying-num"><?php _e( '0 items', 'bu-section-editing' ); ?></span>
